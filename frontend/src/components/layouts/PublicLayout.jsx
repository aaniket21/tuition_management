...
const PublicLayout = ({ children }) => {
    return (
        <div className="min-h-screen flex flex-col font-sans bg-white selection:bg-blue-100 selection:text-blue-900">
            <header className="bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm h-16 flex items-center justify-between px-8 sticky top-0 z-50">
                <Link to="/" className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">
                    Tuition Center
                </Link>
                <nav className="flex gap-8 items-center">
                    ...
                    <Link to="/login" className="px-5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:shadow-lg hover:shadow-blue-500/30 font-medium transition-all hover:-translate-y-0.5">
                        Log In
                    </Link>
                </nav>
            </header>
            <main className="flex-1 bg-gray-50 flex flex-col relative w-full">
                {children}
            </main>
